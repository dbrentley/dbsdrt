...
int verbose_set_frequency(SoapySDRDevice *dev, uint32_t frequency,
    ...
    r = (int)SoapySDRDevice_setFrequency(dev, SOAPY_SDR_RX, channel,
                                         (double)frequency, &args);
    if (r != 0) {
...
int verbose_set_sample_rate(SoapySDRDevice *dev, uint32_t samp_rate,
                            size_t channel) {
    int r;
    r = (int)SoapySDRDevice_setSampleRate(dev, SOAPY_SDR_RX, channel,
                                          (double)samp_rate);
    if (r != 0) {
...
int verbose_set_bandwidth(SoapySDRDevice *dev, uint32_t bandwidth,
                          size_t channel) {
    int r;
    r = (int)SoapySDRDevice_setBandwidth(dev, SOAPY_SDR_RX, channel,
                                         (double)bandwidth);
    ...
    if (r != 0) {
        ...
        applied_bw = (uint32_t)SoapySDRDevice_getBandwidth(dev, SOAPY_SDR_RX,
                                                           channel);
        if (applied_bw)
...
int verbose_offset_tuning(SoapySDRDevice *dev) {
    ...
    if (set_value == NULL) {
        ...
        if ( r == -2 )
                fprintf(stderr, "WARNING: Failed to set offset tuning: tuner
        doesn't support offset tuning!\n"); else if ( r == -3 ) fprintf(stderr,
        "WARNING: Failed to set offset tuning: direct sampling not combinable
        with offset tuning!\n"); else
        */
...
int verbose_auto_gain(SoapySDRDevice *dev, size_t channel) {
    ...
    r = 0;
    /* TODO: not bridged,
    https://github.com/pothosware/SoapyRTLSDR/search?utf8=✓&q=rtlsdr_set_tuner_gain_mode
    ...
    */
    ...
    if (strcmp(driver, "RTLSDR") == 0) {
        ...
        // Note: 26.5 dB in
        // https://github.com/librtlsdr/librtlsdr/blob/master/src/tuner_r82xx.c#L1067
        // - but it's not the same
        // TODO: remove or change after auto-gain?
        // https://github.com/pothosware/SoapyRTLSDR/issues/21
        ...
        // HackRF has three gains LNA, VGA, and AMP, setting total distributes
        // amongst, 116.0 dB seems to work well, even though it logs
        // HACKRF_ERROR_INVALID_PARAM?
        // https://github.com/rxseger/rx_tools/issues/9 Total gain is
        // distributed amongst all gains, 116 = 37,65,1; the LNA is OK (<40) but
        // VGA is out of range (65 > 62)
        // TODO: generic means to set all gains, of any SDR? string parsing
        // LNA=#,VGA=#,AMP=#?
        r = (int)SoapySDRDevice_setGainElement(dev, SOAPY_SDR_RX, channel,
                                               "LNA", 40.); // max 40
        if (r != 0) {
    ...
    */
    ...
    if (strchr(gain_str, '=')) {
        ...
        for (size_t i = 0; i < args.size; ++i) {
            ...
            r = SoapySDRDevice_setGainElement(dev, SOAPY_SDR_RX, channel, name,
                                              value);
            if (r != 0) {
...
int verbose_ppm_set(SoapySDRDevice *dev, int ppm_error, size_t channel) {
    ...
    r = SoapySDRDevice_setFrequencyCorrection(dev, SOAPY_SDR_RX, channel,
                                              (double)ppm_error);
    if (r != 0) {
...
static void show_device_info(SoapySDRDevice *dev) {
    ...
    for (int c = 0; c < num_channels; ++c) {
        ...
        frequencies =
                SoapySDRDevice_listFrequencies(dev, direction, channel, &len);
        ...
        bandwidths =
                SoapySDRDevice_listBandwidths(dev, direction, channel, &len);
...
int suppress_stdout_start(void) {
    // Hack to redirect stdout to stderr so it doesn't interfere with audio
    // output on stdout see
    // https://github.com/rxseger/rx_tools/pull/11#issuecomment-233168397
...
int verbose_setup_stream(SoapySDRDevice *dev, SoapySDRStream **streamOut,
    ...
    if (SoapySDRDevice_setupStream(dev, streamOut, SOAPY_SDR_RX, format,
                                   &channel, 1, &stream_args) != 0) {
        fprintf(stderr, "SoapySDRDevice_setupStream failed: %s\n",
                SoapySDRDevice_lastError());
        return -3;
    }
#else
    *streamOut = SoapySDRDevice_setupStream(dev, SOAPY_SDR_RX, format, &channel,
                                            1, &stream_args);
    if (*streamOut == NULL) {
        fprintf(stderr, "SoapySDRDevice_setupStream failed: %s\n",
                SoapySDRDevice_lastError());
